...
Return exactly 30 events max, ranked by popularity/hype. Real links only."""
        ...
        for attempt in range(3):
            try:
                ...
                print(f"   Response blocks: {len(message.content)}")
                full_text = ""
                for block in message.content:
                    ...
                    if hasattr(block, 'text') and block.text:
                        ...
                        print(f"   Text preview: {block.text[:300]}")
                start_idx = full_text.find("[")
                end_idx = full_text.rfind("]") + 1
                if start_idx != -1 and end_idx > start_idx:
                    events = json.loads(full_text[start_idx:end_idx])
                    print(f"   Found {len(events)} top shows")
                    results.extend(events)
                else:
                    print(f"   No JSON found. Full response: {full_text[:500]}")
                break
            except Exception as ex:
                print(f"   Attempt {attempt+1} failed: {type(ex).__name__}: {ex}")
                if attempt < 2:
                    time.sleep(30)
